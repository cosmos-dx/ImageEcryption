...
class Main_Window(QMainWindow, QWidget, ui,encrypt_page,decrypt_page):
    ...
    def on_click(self):
        ...
        if color.isValid():
            self.clr = color.name()
            m.setStyleSheet("background-color : %s"%self.clr)
    ...
    def on_click1(self):
        ...
        if color.isValid():
            self.clr = color.name()
            self.pushButton.setStyleSheet("background-color : %s"%self.clr)
            self.pushButton_2.setStyleSheet("background-color : %s"%self.clr)
            self.pushButton_8.setStyleSheet("background-color : %s"%self.clr)
            self.pushButton_7.setStyleSheet("background-color : %s"%self.clr)

            
    def Handel_Buttons(self):
        self.sysclr = QPushButton('System Color', self)
        self.sysclr.setToolTip('System Color')
        self.sysclr.move(10,10)
        self.sysclr.clicked.connect(self.on_click)

        self.btnclr = QPushButton('Button Color', self)
        self.btnclr.setToolTip('Button Color')
        self.btnclr.move(10,40)
        self.btnclr.clicked.connect(self.on_click1)
